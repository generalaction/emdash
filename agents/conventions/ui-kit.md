...
The desktop app has exactly one component kit: **`@emdash/ui`** (`packages/ui`). Do not create a
parallel feature-local kit or shim retired component and styling interfaces.
...
- **Icons use the UI ownership seams.** Pass statically imported SVG components to `Icon`,
  reserve `IconSlot` for opaque caller elements, and use `Devicon` or a dedicated rooted
  foreign adapter for external graphic markup. Do not pass runtime Lucide names or add SVG
  descendant helper classes.
...
- `@emdash/theme` owns the literal `tokens` tree, Profile Definitions, generated Token Values,
  manifests, and complete Theme resolution. `@emdash/ui` consumes these interfaces without
  re-exporting Tokens.
- Every host loads `@emdash/ui/styles.css` exactly once before its vendor and host CSS. TypeScript
  and React imports never load CSS.
- Host-authored regions use `surface()` from `@emdash/ui/styles/recipes/surface`;
  React composition may use `<Surface>`. Both require an explicit Level, Role,
  Tone, or context-relative emphasis.
- Shared interactive visuals come from `control()` at
  `@emdash/ui/styles/recipes/control`; components expose semantic props and put
  caller `className` on their documented rendered root.
- Text-entry and form-trigger visuals come from `fieldControl()` at
  `@emdash/ui/styles/recipes/field-control`. InputGroup-style compositions own
  state on their root and expose named child slots; there is no public bare
  input or internal class-map escape hatch.
- Menu/listbox rows use public `menuItem()` from
  `@emdash/ui/styles/recipes/menu-item`; it owns interaction and selection
  states. Overlay shell styling stays private to the owning UI components.
- The controlled Theme runtime applies one Color scheme, Density, and Typography class set.
  React modules and Theme providers never load CSS as a side effect.
- Desktop host rules and Tailwind targets use canonical `--em-*` values in `emdash.host`;
  do not add a host Token palette.
- Package CSS such as Xterm, Devicon, KaTeX, and Chat UI is imported only by the host
  vendor entry in `emdash.vendor`.
- The ownership and runtime flow are documented in
  [Styling architecture](../architecture/styling.md); authoring rules and commands are in
  [UI styling conventions](ui-styling.md).
