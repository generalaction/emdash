...
export function adoptRun(
  ...
  const promise = definitionMutex
    .runExclusive(automationId, () => adoptRunOnce(dependencies, automationId, runId))
...
async function adoptRunOnce(
  ...
  const target = await resolveAutomationRuntime(dependencies.runtime, projectId);
  ...
  await upsertRunProjection(dependencies.db, runtimeRun);
