...
import { getSourceControlClient } from '@core/features/source-control/browser/client';
...
export class MonacoModelRegistry {
  ...
  private runtimePath(
    ...
    workspaceId: string,
    filePath: string
  ): { workspaceId: string; relative: PortableRelativePath } {
    ...
    return editorFilePath(workspaceId, workspace.path, filePath);
  }
  ...
  private applyDiskUpdate(
    diskUri: string,
    entry: DiskModelEntry,
    content: EditorFileContentModel
  ): void {
