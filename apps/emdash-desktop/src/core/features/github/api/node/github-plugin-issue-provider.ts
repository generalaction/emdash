...
export type GitHubIssueProviderDependencies = {
...
async function resolveGitHubPluginCredentials(
  ...
  const token = await dependencies.auth.getToken(repository.host, authContext);
...
export function createGitHubPluginIssueProvider(
  plugin: IssuesPluginProvider,
  dependencies: GitHubIssueProviderDependencies
): IssueProvider {
  const capabilities = toIssueProviderCapabilities(plugin);
  const pluginLog = dependencies.logger.child({ integration: 'github' });
  ...
  async function invoke(
    ...
    const authContext = await resolveIssueAuthContext(dependencies, opts.projectId);
    if (!authContext.success) return err(authContext.error);

    const credentials = await resolveGitHubPluginCredentials(
      dependencies,
      repository.data,
      authContext.data
    );
