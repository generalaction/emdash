...
} from '../../node/create-project-provider';
...
export class ProjectSessionManager implements Hookable<ProjectSessionManagerHooks>, Disposable {
  ...
  >({
    ...
    onObserverError: ({ error }) =>
      log.error('ProjectManager: lifecycle observer error', { error }),
  });

  constructor(private readonly dependencies: CreateProjectProviderDependencies) {}
  ...
  private async startProject(
    ...
    try {
      const provider = await runWithTimeout(() => createProvider(this.dependencies, project), {
