...
} from '@core/features/tasks/api/node/resolve-workspace-intent';
import {
  buildTaskFromWorkspace,
  emitTaskProvisionProgress,
} from '@core/features/tasks/api/node/task-builder';
...
export class WorkspaceBootstrapService {
  ...
  private async compileRuntimeWorkspacePlan(
    ...
    const worktreePoolPath = await this.dependencies.placement.resolveWorktreePool(project.project);
  ...
  private async _acquireAndBuild(
    ...
    const workspaceIdentity = this.dependencies.workspaceIdentity;
    workspaceIdentity.invalidate(workspaceId);
    const accessResult = await tryAcquireWorkspaceRuntime(
