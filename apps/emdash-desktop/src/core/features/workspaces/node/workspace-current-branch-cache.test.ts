...
const db = { select: dbMocks.select, update: dbMocks.update } as never;
...
describe('refreshWorkspaceCurrentBranchCache', () => {
  ...
  it('writes the cache and reports changed when the branch differs', async () => {
    ...
    const result = await refreshWorkspaceCurrentBranchCache(db, 'ws-1', () =>
  ...
  it('returns undefined when the workspace is not found', async () => {
    ...
    const result = await refreshWorkspaceCurrentBranchCache(db, 'missing', () =>
      Promise.resolve('any')
    );
