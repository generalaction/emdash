...
import type {
  GitPlatformProvider,
  GitPlatformPullRequestRepository,
} from '../git-platform-provider';
import { mapApiError, mapGitHubAuthError } from './github-provider-errors';
import {
...
export class GitHubProvider implements GitPlatformProvider {
  ...
  // Single-provider invariant: this module is the only place that constructs a
  // PullRequestMetadata, so this literal is always correct. A second provider must
  // never call this mapper — it produces its own.
