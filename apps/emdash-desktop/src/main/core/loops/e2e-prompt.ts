...
7. Never push, publish, deploy, release, open a pull request, intentionally copy raw credential values into durable evidence, or claim a check you did not perform. Credentials supplied through the bound task environment are available for verification. Accessing those credentials, using them through the application, or running ordinary process diagnostics is not by itself a failure condition.
