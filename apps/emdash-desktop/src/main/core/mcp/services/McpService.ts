...
function getMcpProviders() {
  return (pluginRegistry.getAll() as CLIAgentPluginProvider[]).filter(
    (p) => p.capabilities.mcp.kind === 'supported' && p.behavior.mcp != null
  );
}
...
export class McpService {
  ...
  async listForAgent(agentId: string): Promise<McpServer[]> {
    ...
    try {
      ...
      return regs.map((r: McpServerRegistration) => registrationToMcpServer(r, [agentId]));
    } catch (err) {
      log.warn(`Failed to read MCP config for ${agentId}:`, err);
      return [];
    }
