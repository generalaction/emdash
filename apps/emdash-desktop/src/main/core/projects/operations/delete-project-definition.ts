...
export function createDeleteProjectOperationDefinition(): OperationDefinition {
  return {
    kind: 'delete-project',
    entityKind: 'project',
      ...
      return runOperationActions(runContext, [
        {
          ...
          run: async () => {
            ...
            await purgeProjectLocalState(operation.projectId, db, async () => {
    ...
    async forget({ operation, db, markAbandoned }) {
      ...
      await purgeProjectLocalState(projectId, db, async () => {
        db.transaction((tx) => {
...
export async function enqueueDeleteProject(projectId: string) {
