...
import { resolveLifecycleOperationContext } from './lifecycle-operation-context';
...
export function createDeleteWorkspaceOperationDefinition(): OperationDefinition {
  return {
    kind: 'delete-workspace',
    entityKind: 'workspace',
    describe: describeWorkspaceOperation,
    async run(runContext) {
      ...
      addSessionActions(actions, runContext, context, targets);
      actions.push(
        {
        ...
        },
        {
          ...
          run: async () => purgeLifecycleWorkspaceRow(db, operation, context),
        }
      );
...
export function createArchiveWorkspaceOperationDefinition(): OperationDefinition {
  return {
      ...
      if (context.workspacePath) {
        actions.push(
          ...
          },
          {
            id: 'clean-artifacts',
            timeoutMs: WORKSPACE_TIMEOUT_MS,
            run: async () => cleanLifecycleWorkspaceArtifacts(operation, context),
          }
        );
      }
      actions.push({
...
export async function enqueueDeleteWorkspace(workspaceId: string) {
...
export async function enqueueDeleteWorkspacePath(input: ArchiveWorkspaceInput) {
  return getOperationsEngine().submit(async ({ db, clock }) => {
...
export async function enqueueArchiveWorkspace(input: ArchiveWorkspaceInput) {
  return getOperationsEngine().submit(async ({ db }) => {
    const [project] = await db
      .select()
      .from(projects)
...
async function describeWorkspaceOperation({
  operation,
  db,
}: Parameters<OperationDefinition['describe']>[0]) {
  const context = await resolveLifecycleOperationContext(db, operation);
  return {
...
function addSessionActions(
  ...
  targets: Awaited<ReturnType<typeof resolveLifecycleSessionTargets>>
): void {
