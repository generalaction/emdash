...
function AutomationsHeroGraphic() {
  return (
    <div className="pointer-events-none w-60 rounded-lg bg-white p-1 text-xs shadow-2xl ring-1 ring-black/10 select-none">
...
/**
 * Resolves the announcement media area. Remote `image` URLs take precedence over
 * built-in coded `hero` components shipped with the app.
 */
...
export function FeatureAnnouncementMediaArea({
  ...
  if (media.kind === 'image') {
    return (
      <div className={cn('relative shrink-0 overflow-hidden bg-neutral-950', heightClass)}>
        <img
          src={media.url}
          alt=""
          className="absolute inset-0 size-full object-cover object-center"
        />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(0,0,0,0.15),transparent_70%)]" />
      </div>
    );
  }
  ...
  return (
    <div
      className={cn(
        'relative flex shrink-0 items-center justify-center overflow-hidden bg-neutral-950',
        heightClass
      )}
    >
