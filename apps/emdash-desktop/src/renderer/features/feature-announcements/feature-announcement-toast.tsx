...
import { FEATURE_ANNOUNCEMENT_TOASTER_ID } from '@renderer/features/feature-announcements/feature-announcement-toaster';
...
export function showFeatureAnnouncementToast(
  ...
  toast.custom(
    (id) => <FeatureAnnouncementToastCard manifest={manifest} toastId={id} options={options} />,
    {
      ...
      toasterId: FEATURE_ANNOUNCEMENT_TOASTER_ID,
    }
  );
}
      ...
      <div className="flex flex-col gap-3 p-4">
        ...
        <ul className="flex flex-col gap-2.5">
          {manifest.features.map((feature, index) => (
            <li key={`${feature.title}-${index}`} className="min-w-0 text-xs">
