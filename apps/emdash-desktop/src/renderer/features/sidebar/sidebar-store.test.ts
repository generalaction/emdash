...
function unmountedProject(
  id: string,
  name: string,
  createdAt = '2026-01-01T00:00:00.000Z'
): ProjectStore {
  const data = localProject(id, name, createdAt);
  return {
    state: 'unmounted',
    id,
    name,
...
describe('SidebarStore project ordering', () => {
  it('sorts projects newest first by default', () => {
    const store = createStore([
      unmountedProject('old', 'Old', '2026-01-01T00:00:00.000Z'),
      unmountedProject('new', 'New', '2026-01-02T00:00:00.000Z'),
    ]);
  ...
  it('places projects missing from a saved manual order first', () => {
    const store = createStore([
      unmountedProject('old', 'Old', '2026-01-01T00:00:00.000Z'),
      unmountedProject('manual', 'Manual', '2026-01-02T00:00:00.000Z'),
      unmountedProject('new', 'New', '2026-01-03T00:00:00.000Z'),
    ]);
