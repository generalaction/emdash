  ...
  implements Snapshottable<TabManagerSnapshot>, TabHost
{
  ...
  private readonly _layoutOpener:
    | ((kind: string, args: unknown, config?: TabOpenOptions) => void)
    | null;
  ...
  constructor(
    ...
    ctx: TabViewContext,
    opts?: {
      layoutOpener?: (kind: string, args: unknown, config?: TabOpenOptions) => void;
   ...
   */
  openKind(kind: string, args: unknown, config?: TabOpenOptions): void {
  ...
  commitRename(tabId: string, name: string): void {
    ...
    def.commands.rename.exec(resource as never, name);
  }
