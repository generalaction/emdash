...
vi.mock('../agent-config/runtime-client', () => ({
  getAgentConfigRuntimeClient: () => Promise.resolve(runtimeClient.current),
}));

vi.mock('@emdash/wire', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@emdash/wire')>();
  return {
    ...actual,
    ReplicaState: class {
...
describe('AcpAuthLoginBinding', () => {
  ...
  it('cancels login once during idempotent disposal', async () => {
    ...
    expect(client.cancelLogin).toHaveBeenCalledWith({ providerId: 'provider' });
  });
