...
function wave5PlanSource(): string {
  return `Execute Wave 5 of the approved ACP Loops v2 plan against this Emdash-provisioned Summario worktree. The source checkout is read-only. Use Codex gpt-5.6-sol and a fresh ACP conversation for every ordered phase. Follow AGENTS.md and every referenced repository instruction. The lead exclusively owns the fresh non-production Convex backend, lifecycle processes, preview origin, fixtures, credential provisioning, and cleanup; implementation sessions must not change credential configuration or run Convex deployment/codegen commands. Credentials in the exact bound verification workspace are available to the verification agent. The agent may read them at runtime and enter them through the application when authentication is required; credential access, application use, or ordinary process diagnostics are not automatic failures. Never print credential values or persist them in repository files, prompts, logs, screenshots, or evidence. The bound local backend intentionally has no live Google OAuth client. For OAuth draft-restoration acceptance, do not invoke drive:startOAuth or leave the exact allowed origin; persist the wizard draft, then use same-origin drive=error or return-query navigation and guarded local/test fixtures as needed to emulate the OAuth return and prove restoration without console or network errors. Required backend tests cover the live OAuth contract. The latest exact-checkpoint native attempt passed every validation but opened on an empty Meetings screen where New meeting was disabled and the responsive Templates target was unavailable, so the guarded vocabulary fixture could not be exercised. Reproduce this fresh-state reachability defect and implement the smallest guarded agent-test-user vocabulary fixture surface, following the existing app/agent fixture-page conventions and reusing browserFixtures.prepareVocabularyExtraction. It must support a deterministic same-origin /agent/vocabulary-fixtures?prepare=1 journey that prepares the marker-owned fixture only when existing backend guards allow it, then navigates to the returned edit route; normal users and fixture-disabled environments must remain rejected by the existing mutation. Add focused coverage where an existing harness fits, commit the correction in the disposable clean room, and return the correction-ready sentinel so Emdash can integrate it and require a destroyed-and-recreated fresh replay. On the next unchanged replay, begin native vocabulary acceptance at /agent/vocabulary-fixtures?prepare=1 instead of relying on Meetings or responsive navigation. For focused Vitest work, inspect the repository scripts first and invoke pnpm exec vitest run with explicit test-file paths; never pass focused paths through pnpm test because that script starts the repository-wide suite. Keep changes minimal and modular, use TDD, respect each phase's file ownership, preserve manual vocabulary and transcription-context behavior, and never push or open a PR. Terminal Review must verify authorization, cross-user rejection, normalization and limits, guarded-fixture denial paths, draft versioning/default-on behavior, UX states, reference removal, documentation accuracy, simplicity, clean git state, and absence of intentionally persisted raw credentials. Independent clean-room E2E must use the exact replayed checkpoint and Emdash native preview/browser verifier; it must prove default-on, opt-out, loading/failure/retry, editable/removable suggestions, persistence across back navigation and OAuth draft restore, manual-term preservation, edit flow, completed-Meeting absence, and continued transcription-context use, with no console/network errors. Run pnpm test, pnpm exec tsc --noEmit, pnpm lint, and pnpm build. Do not claim success unless every phase, Review, native clean-room E2E, replay, and required gate is green.`;
}
