...
function wave6WorkPhases(privacyMode: Wave6LegalFacts['privacyMode']) {
  return [
    ...
    },
    {
      ...
      goal: `Close the approved transcript-deletion truth gap with failing integration tests first. Own only convex/meetings.ts, convex/lib/transcriptStorage.ts, convex/cronActions.ts, convex/crons.ts, and convex/transcriptStorage.integration.test.ts plus a narrowly required shared deletion helper. Extract one ownership-safe document-plus-blob cleanup path and prove both public meetings.remove and cron-driven internalDelete remove the meeting document and referenced transcript-storage blob without weakening authorization or idempotency. Do not run Convex deployment or codegen commands. Commit only this phase's coherent files.`,
    },
    {
