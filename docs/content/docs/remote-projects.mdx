...
description: Run agents on remote machines over SSH
---

Run agents on a remote server over SSH. Instead of working only with local projects, you can connect to any machine and orchestrate agents against content hosted there.

<img
  src="/media/addremoteproject.png"
  alt="Add Remote Project in Scrawl"
  style={{
    width: '100%',
    maxWidth: '700px',
    margin: '20px auto',
...
If you have an existing `~/.ssh/config`, Scrawl detects it automatically. Select a host from the dropdown to pre-fill the form.
...
| Method          | Description                                                                                      |
| --------------- | ------------------------------------------------------------------------------------------------ |
| **SSH Agent**   | Uses your system SSH agent. No keys stored in Scrawl. Recommended.                               |
...
If SSH agent auth fails, Scrawl suggests falling back to key-based auth and shows common key paths.
...
Scrawl auto-connects when you select a remote project and monitors the connection with periodic health checks. If the connection drops, it reconnects automatically with exponential backoff (up to 3 attempts).
...
4. Terminal output streams back to your local Scrawl UI in real time

All shell commands are executed on the remote server. Your local machine only runs the Scrawl interface.
...
See the [SSH Setup Guide]({GITHUB_REPO}/blob/main/docs/ssh-setup.md) for detailed server preparation steps.
...
Cloud-hosted team environments with centralized configuration and admin controls are on the Scrawl roadmap.
