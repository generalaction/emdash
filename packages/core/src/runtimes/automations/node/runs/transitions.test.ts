...
describe('AutomationRunTransitions', () => {
  ...
  it('walks the happy path and claims a fresh seq on every transition', () => {
    ...
    const claimed = transitions.claimQueued('run-1', 2000);
    ...
    expect(done?.finishedAt).toBe(3000);
  ...
  it('reports every successful transition and nothing on a lost race', () => {
    ...
    transitions.claimQueued('run-1', 2000);
    ...
    expect(transitions.claimQueued('run-1', 2500)).toBeNull();
    expect(changed).toHaveLength(2);
  });
  it('returns null when the source status does not match', () => {
    runStore.insertRun(newRun());
    expect(transitions.markDone('run-1', { conversationId: 'c', sessionId: 's' }, 3000)).toBeNull();
  ...
  it('fails a run from any non-terminal status but never from a terminal one', () => {
    ...
    expect(transitions.markFailed('run-1', { step: 'run', code: 'cancelled' }, 4000)).toBeNull();
  });
  it('skips runs only before a worker has claimed them', () => {
    ...
    runStore.insertRun(newRun({ id: 'run-2', scheduledAt: 1500 }));
    transitions.markQueued('run-2');
    transitions.claimQueued('run-2', 2000);
    expect(
      transitions.markSkipped('run-2', { step: 'queue', code: 'automation_deleted' }, 2500)
    ).toBeNull();
  });
  it('cancels queued and in-flight runs but never terminal runs', () => {
    runStore.insertRun(newRun());
    transitions.markQueued('run-1');
    const cancelled = transitions.markCancelled('run-1', 2000);
    expect(cancelled?.status).toBe('cancelled');
    expect(cancelled?.finishedAt).toBe(2000);
    expect(transitions.claimQueued('run-1', 2500)).toBeNull();
    expect(transitions.markCancelled('run-1', 3000)).toBeNull();
    runStore.insertRun(newRun({ id: 'run-2', scheduledAt: 1500 }));
    transitions.markQueued('run-2');
    transitions.claimQueued('run-2', 2000);
    expect(transitions.markCancelled('run-2', 2500)?.status).toBe('cancelled');
  });
  it('interrupts in-flight runs with the step matching their stuck status', () => {
    ...
    const interrupted = transitions.markInterrupted(stuck, 3000);
  ...
  it('rejects interrupting runs that are not in-flight', () => {
    ...
    expect(() => transitions.markInterrupted(inserted, 3000)).toThrow(/not in-flight/);
  });
});
