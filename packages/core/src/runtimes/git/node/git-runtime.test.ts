...
describe('GitRuntime', () => {
  ...
  it('pushes an initial commit after cloning when initialize is requested', async () => {
    const source = await mkdtemp(path.join(tmpdir(), 'emdash-shared-runtime-clone-bare-'));
    await execFileAsync('git', ['init', '--bare', '-b', 'main'], { cwd: source });

    const parent = await mkdtemp(path.join(tmpdir(), 'emdash-shared-runtime-clone-init-'));
    const target = path.join(parent, 'repo');
    const runtime = new GitRuntime({
      env: {
    ...
    try {
      const result = await runtime.provisioning.cloneRepository(
        source,
        hostPath(target),
        { onProgress: (progress) => phases.push(progress.phase) },
        { initialize: { name: 'My Project', description: 'A test project' } }
      );
      ...
      await expect(readFile(path.join(target, 'README.md'), 'utf8')).resolves.toBe(
        '# My Project\n\nA test project\n'
      );
      const { stdout } = await execFileAsync('git', ['log', '--format=%s', 'main'], {
        cwd: source,
      });
      expect(stdout.trim()).toBe('Initial commit');
      expect(phases).toContain('initialize');
    } finally {
      await runtime.dispose();
    }
  });

  it('writes a name-only README when initialize has no description', async () => {
    const source = await mkdtemp(path.join(tmpdir(), 'emdash-shared-runtime-clone-bare2-'));
    await execFileAsync('git', ['init', '--bare', '-b', 'main'], { cwd: source });

    const parent = await mkdtemp(path.join(tmpdir(), 'emdash-shared-runtime-clone-init2-'));
    const target = path.join(parent, 'repo');
    const runtime = new GitRuntime({
      env: {
        GIT_AUTHOR_NAME: 'Test User',
