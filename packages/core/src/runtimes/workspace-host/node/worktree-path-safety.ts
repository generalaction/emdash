 ...
 *
 * Workspace-host never recursively removes an unregistered stale path, so the old
...
): Promise<Result<void, WorkspaceHostError>> {
...
function unsafe(targetPath: string, reason: string): Result<never, WorkspaceHostError> {
  return err({
...
function foreign(targetPath: string): Result<never, WorkspaceHostError> {
  return err({
    type: 'operation-rejected',
