...
import { fieldControlAnatomy } from '@styles/recipes/field-control.css';
...
> {
  ...
  /** Removes the standalone field shell for use on an existing owning surface. */
  bare?: boolean;
...
const SearchInput = React.forwardRef<HTMLInputElement, SearchInputProps>(function SearchInput(
  ...
  const controlClassName = cx(
    styles.control({ bare, clearable: onClear != null, size }),
    inputClassName
  );
  ...
  const control = bare ? (
    <InputPrimitive
      ...
      data-tone={tone}
      value={value}
      className={cx(fieldControlAnatomy({ size }), styles.bareControl, controlClassName)}
      style={inputStyle}
      onKeyDown={(event) => {
        if (event.key === 'Escape') {
  ...
  return (
    <div data-slot="search-input" className={cx(styles.container, className)} style={style}>
      <Icon source={SearchIcon} className={styles.icon({ bare })} />
      {control}
