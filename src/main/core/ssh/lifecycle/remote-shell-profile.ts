...
import {
  ...
  terminalShellCommandFlag,
  terminalShellEnvCaptureFlag,
...
export function buildRemoteShellCommand(
  ...
  return `${quoteShellArg(shell)} ${terminalShellCommandFlag(shell)} ${quoteShellArg(
    `${prefix}${command}`
  )}`;
}
...
export function buildRemoteShellCommandWithPathLookup(
  ...
  const path = env.PATH ?? profile.env.PATH;
  const pathArg = path ? `${quoteShellArg(`PATH=${path}`)} ` : '';
  return `${quoteShellArg('/usr/bin/env')} ${pathArg}${quoteShellArg(shellName)} ${terminalShellCommandFlag(
    shellName
  )} ${quoteShellArg(`${prefix}${command}`)}`;
}
...
async function captureRemoteEnv(
  ...
  try {
    ...
    const capture = `${guard}${quoteShellArg(shell)} ${terminalShellEnvCaptureFlag(
      shell
    )} ${quoteShellArg('env')}`;
