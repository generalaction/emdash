...
const AppContent: React.FC = () => {
  ...
  const handleCreateTask = async (
    ...
    try {
      ...
      // Seed conversation with issue context if applicable
      // Only create/get conversation once, not multiple times
      {
      ...
      // Track task creation
      const { captureTelemetry } = await import('./lib/telemetryClient');
      const isMultiAgentTelemetry = (newTask.metadata as any)?.multiAgent?.enabled;
      captureTelemetry('task_created', {
        provider: isMultiAgentTelemetry ? 'multi' : (newTask.agentId as string) || 'codex',
        has_initial_prompt: !!taskMetadata?.initialPrompt,
      });

      // Set the active task and its agent (none if multi-agent)
