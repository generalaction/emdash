  ...
  const handleDeleteTask = async (
    ...
    const runDeletion = async (): Promise<boolean> => {
      try {
        ...
        const removeResult = shouldRemoveWorktree ? results[0] : null;

        // Check worktree removal result (if applicable)
        if (shouldRemoveWorktree && removeResult) {
        ...
        const remoteBranchWarning =
          !!options?.deleteRemoteBranch && removeResult?.status === 'fulfilled'
            ...
            : false;
        const remoteBranchDeleteError =
          removeResult && removeResult.status === 'fulfilled'
