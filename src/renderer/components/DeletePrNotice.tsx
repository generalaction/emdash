...
export const DeletePrNotice: React.FC<Props> = ({ tasks, gitPlatform }) => {
  ...
  return (
    <div className="space-y-2 rounded-md border border-amber-300/60 bg-amber-50 px-3 py-2 text-sm text-amber-900 dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-50">
      <div className="flex items-center gap-2">
        ...
        <p className="font-medium">Open {labels.prNoun} detected</p>
      </div>
      <div className="space-y-1">
        {tasks.map((ws) => {
          const badge = ws.pr.isDraft ? 'Draft' : labels.prNoun;
          ...
          return (
            ...
            >
              ...
              {hasValidPr ? (
                <PrPreviewTooltip pr={ws.pr as any} gitPlatform={gitPlatform}>
                  {prBadge}
                </PrPreviewTooltip>
              ) : (
                prBadge
