...
const TaskModal: React.FC<TaskModalProps> = ({
  ...
  const handleNameChange = (val: string) => {
    ...
    // Track custom naming for telemetry
    if (autoGeneratedName && val !== autoGeneratedName && val.trim()) {
      void (async () => {
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && !isCreating && onClose()} modal={false}>
      <DialogContent className="max-h-[calc(100vh-48px)] max-w-md overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {providerRuns.length > 0 && providerRuns.every((pr) => pr.provider === 'claude') && (
            <div className="flex items-center gap-3 rounded-md border border-dashed border-border/60 bg-muted/30 px-3 py-2">
              <Checkbox
                id="debate-mode"
                checked={debateMode}
                onCheckedChange={(checked) => setDebateMode(checked === true)}
              />
              <div className="flex-1">
                <Label htmlFor="debate-mode" className="cursor-pointer text-sm font-medium">
                  Debate mode
