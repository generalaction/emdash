...
const TaskModal: React.FC<TaskModalProps> = ({
  ...
  // Validation — empty is allowed (a random name is generated at submit time)
  const validate = useCallback(
    (value: string): string | null => {
      const normalized = normalizeTaskName(value);
      if (!normalized) return null; // empty is fine — will auto-generate on submit
  ...
  useEffect(() => {
    ...
    window.electronAPI.getSettings().then((res) => {
      ...
      setAutoGenerateName(settings?.tasks?.autoGenerateName !== false);
    });
  ...
  // Regenerate name when initial prompt or linked issue changes (including removal)
  useEffect(() => {
    ...
    if (generated) {
      const unique = ensureUniqueTaskName(generated, normalizedExisting);
      setTaskName(unique);
      setAutoGeneratedName(unique);
      setError(validate(unique));
    } else {
      setTaskName('');
      setAutoGeneratedName('');
      setError(null);
    }
  }, [autoGenerateName, initialPrompt, selectedLinearIssue, selectedGithubIssue, selectedJiraIssue, normalizedExisting, validate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If the user left the name empty, generate a random friendly name
    const normalized = normalizeTaskName(taskName);
    const finalName = normalized || generateFriendlyTaskName(normalizedExisting);
    const isAutoNamed = autoGenerateName && !userHasTypedRef.current;
