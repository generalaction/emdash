...
const TaskModal: React.FC<TaskModalProps> = ({
  ...
  const inferNameFromContext = useCallback(
      ...
      inferTaskNameFromContext(
        {
          ...
          githubIssue: selectedGithubIssue
            ? {
                number: selectedGithubIssue.number,
                title: selectedGithubIssue.title,
              }
            : null,
          jiraIssue: selectedJiraIssue
            ? {
                key: selectedJiraIssue.key,
                summary: selectedJiraIssue.summary,
              }
            : null,
        },
        normalizedExisting,
    ...
    [initialPrompt, normalizedExisting, selectedGithubIssue, selectedJiraIssue, selectedLinearIssue]
  );
  ...
  const validate = useCallback(
    (value: string): string | null => {
      ...
      if (!normalized) {
        if (isAutoGenerateEnabled) return null;
    ...
    [isAutoGenerateEnabled, normalizedExisting]
  );
  ...
  useEffect(() => {
    ...
    const suggested = inferNameFromContext(generateFriendlyTaskName(normalizedExisting));
    ...
    window.electronAPI.getSettings().then((res) => {
      ...
      const autoGenerateName = settings?.tasks?.autoGenerateName !== false;
      setIsAutoGenerateEnabled(autoGenerateName);

      if (!autoGenerateName && !userHasTypedRef.current) {
        setAutoGeneratedName('');
        setTaskName('');
        setError(null);
        return;
      ...
      if (autoGenerateName && !userHasTypedRef.current) {
        const inferred = inferNameFromContext(suggested);
        setAutoGeneratedName(inferred);
        setTaskName(inferred);
        setError(validate(inferred));
      }
    });
  ...
  }, [defaultBranch, inferNameFromContext, isOpen, normalizedExisting, validate]);

  useEffect(() => {
    if (!isOpen || !autoNameInitializedRef.current || !isAutoGenerateEnabled) {
      return;
    }

    const normalizedCurrent = normalizeTaskName(taskName);
    const normalizedAuto = normalizeTaskName(autoGeneratedName);
    const shouldReplaceCurrent =
      !normalizedCurrent ||
      !userHasTypedRef.current ||
      (Boolean(normalizedAuto) && normalizedCurrent === normalizedAuto);

    if (!shouldReplaceCurrent) {
      return;
    }

    const inferred = inferNameFromContext(
      autoGeneratedName || generateFriendlyTaskName(normalizedExisting)
    );
    if (!inferred || inferred === normalizedCurrent) {
      return;
    }
  ...
  const resolveTaskName = useCallback((): string => {
    const normalizedInputName = normalizeTaskName(taskName);
    ...
    return inferNameFromContext(autoGeneratedName || generateFriendlyTaskName(normalizedExisting));
  }, [autoGeneratedName, inferNameFromContext, isAutoGenerateEnabled, normalizedExisting, taskName]);

  const submitError = useMemo(() => validate(resolveTaskName()), [resolveTaskName, validate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const err = validate(resolvedTaskName);
    if (err) {
      setError(err);
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <Label htmlFor="task-name" className="mb-2 block">
              Task name{isAutoGenerateEnabled ? ' (optional)' : ''}
            </Label>
            <SlugInput
            ...
            {isAutoGenerateEnabled ? (
              <p className="mt-1 text-xs text-muted-foreground">
                Leave blank to auto-infer from prompt or linked issue context.
              </p>
            ) : null}
          </div>
          ...
          <DialogFooter>
            <Button type="submit" disabled={Boolean(submitError)}>
              Create
            </Button>
          </DialogFooter>
        </form>
